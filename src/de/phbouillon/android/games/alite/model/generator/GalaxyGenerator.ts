...
export class GalaxyGenerator {
	...
	private generatePlanetsOfGalaxy(galaxyNumber: number): void {
		...
		if (this.highestHashedGalaxyNumber <= galaxyNumber) {
			...
			for (let systemCount = 0; systemCount < GalaxyGenerator.PLANET_COUNT; systemCount++) {
				const planetName = (galaxyNumber === this.currentGalaxy ? this.system[systemCount].name :
					SystemData.generateRandomName(seed)).toLowerCase();
				const planetId = (galaxyNumber << 8) + systemCount;
				if (!this.planetNames.has(planetName)) {
					this.planetNames.set(planetName, planetId);
				} else if (this.planetNames.get(planetName) !== planetId) {
		...
		if(duplicatesInGalaxy) {
            for (const [index, planetName] of duplicatesInGalaxy.entries()) {
                let newPlanetName: string;
                do {
                    newPlanetName = SystemData.generateRandomName(seed);
                } while (this.planetNames.has(newPlanetName.toLowerCase()));

                if (galaxyNumber === this.currentGalaxy) {
                    this.system[index].changePlanetName(newPlanetName);
                }
                this.planetNames.set(newPlanetName.toLowerCase(), (galaxyNumber << 8) + index);
            }
        }
	}

	public rebuildGalaxy(): void {
		// SystemData.changeLocale(); // This needs to be handled differently
	...
	public findGalaxyOfPlanet(name: string): number {
		...
		try {
			for (let j = 0; j < Settings.maxGalaxies - 1; j++) {
				if (this.planetNames.has(name.toLowerCase())) {
					return this.planetNames.get(name.toLowerCase())! >> 8;
				}
