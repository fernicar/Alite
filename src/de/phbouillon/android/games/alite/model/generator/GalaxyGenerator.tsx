...
export class GalaxyGenerator {
    private static readonly BASE_SEEDS: number[][] = [
    ...
    private static readonly ORPHAN_PLANETS: Set<number> = new Set([
    ...
    public static readonly GALAXY_COUNT = 8;
    public static readonly EXTENDED_GALAXY_COUNT = GalaxyGenerator.BASE_SEEDS.length;
    public static readonly PLANET_COUNT = 256;
    public static readonly ORPHAN_PLANET_COUNT = GalaxyGenerator.ORPHAN_PLANETS.size;

    private readonly system: SystemData[] = new Array(GalaxyGenerator.PLANET_COUNT);
    private currentGalaxy = 1;
    private highestHashedGalaxyNumber = 1;
    private readonly planetNames: Map<string, number> = new Map();
    private readonly duplicates: Map<number, Map<number, string>> = new Map();

    public buildGalaxy(galaxyNumber: number): void {
        this.currentGalaxy = galaxyNumber;
        this.buildGalaxyOf(galaxyNumber);
    }

    private buildGalaxyOf(galaxyNumber: number): void {
        SystemData.initialize();
        const time = Date.now();
        if (this.highestHashedGalaxyNumber <= galaxyNumber) {
            for (let i = this.highestHashedGalaxyNumber; i <= galaxyNumber; i++) {
                this.generatePlanetsOfGalaxy(i);
            }
            this.highestHashedGalaxyNumber = galaxyNumber + 1;
        } else {
            this.generatePlanetsOfGalaxy(galaxyNumber);
        }
        AliteLog.d("Galaxy generation", `Galaxy #${galaxyNumber} generated in ${Date.now() - time} ms`);
    }

    private generatePlanetsOfGalaxy(galaxyNumber: number): void {
        if (galaxyNumber > Settings.maxGalaxies || galaxyNumber > GalaxyGenerator.EXTENDED_GALAXY_COUNT || galaxyNumber < 1) {
            galaxyNumber = 1;
        }
        const seed = new SeedType(GalaxyGenerator.BASE_SEEDS[galaxyNumber - 1][0], GalaxyGenerator.BASE_SEEDS[galaxyNumber - 1][1], GalaxyGenerator.BASE_SEEDS[galaxyNumber - 1][2]);

        if (galaxyNumber === this.currentGalaxy) {
            for (let systemCount = 0; systemCount < GalaxyGenerator.PLANET_COUNT; systemCount++) {
                this.system[systemCount] = SystemData.createSystem(galaxyNumber, systemCount, seed);
            }
        }

        if (this.highestHashedGalaxyNumber <= galaxyNumber) {
            const duplicatesInGalaxy: Map<number, string> = new Map();
            for (let systemCount = 0; systemCount < GalaxyGenerator.PLANET_COUNT; systemCount++) {
                const planetName = (galaxyNumber === this.currentGalaxy ? this.system[systemCount].getName() :
                    SystemData.generateRandomName(seed)).toLowerCase();
                const planet = this.planetNames.get(planetName);
                const planetId = (galaxyNumber << 8) + systemCount;
                if (planet === undefined) {
                    this.planetNames.set(planetName, planetId);
                } else if (planet !== planetId) {
                    duplicatesInGalaxy.set(systemCount, planetName);
                }
            }
            this.duplicates.set(galaxyNumber, duplicatesInGalaxy);
        }

        const duplicatesInGalaxy = this.duplicates.get(galaxyNumber);
        if (duplicatesInGalaxy) {
            for (const [index, name] of duplicatesInGalaxy.entries()) {
                let planetName: string;
                let existsPlanet: number;
                do {
                    planetName = SystemData.generateRandomName(seed);
                    existsPlanet = this.planetNames.get(planetName.toLowerCase());
                } while (existsPlanet !== undefined && existsPlanet !== ((galaxyNumber << 8) + index));

                if (galaxyNumber === this.currentGalaxy) {
                    this.system[index].changePlanetName(planetName);
                }
                this.planetNames.set(planetName.toLowerCase(), (galaxyNumber << 8) + index);
            }
        }
    }


    public rebuildGalaxy(): void {
        SystemData.changeLocale();
        this.buildGalaxy(this.currentGalaxy);
    }

    public getCurrentGalaxy(): number {
        return this.currentGalaxy;
    }

    public setCurrentGalaxy(galaxy: number): void {
        this.currentGalaxy = galaxy;
    }

    public getNextGalaxy(): number {
        if (this.currentGalaxy >= Settings.maxGalaxies || this.currentGalaxy < 1) {
            return 1;
        }
        return this.currentGalaxy + 1;
    }

    public getSystems(): SystemData[] {
        return this.system;
    }

    public getSystem(index: number): SystemData {
        return this.currentGalaxy === SystemData.RAXXLA_GALAXY && index === SystemData.RAXXLA_SYSTEM_INDEX ?
            SystemData.RAXXLA_SYSTEM : this.system[index % GalaxyGenerator.PLANET_COUNT];
    }

    public isOrphan(id: number): boolean {
        return GalaxyGenerator.ORPHAN_PLANETS.has(id);
    }

    public getAveragePrice(tradeGood: TradeGood): number {
        return tradeGood.getAveragePrice(this.currentGalaxy);
    }

    public findGalaxyOfPlanet(name: string): number {
        let i = this.currentGalaxy;
        try {
            for (let j = 0; j < Settings.maxGalaxies - 1; j++) {
                const galaxy = this.planetNames.get(name.toLowerCase());
                if (galaxy !== undefined) {
                    return galaxy >> 8;
                }
                i = i % (Settings.maxGalaxies + 1) + 1;
                if (i <= this.highestHashedGalaxyNumber) {
                    continue;
                }
                AliteLog.d("Analyzing Galaxy", `Analyzing Galaxy ${i}`);
                this.buildGalaxyOf(i);
            }
        } finally {
            if (i !== this.currentGalaxy) {
                this.buildGalaxyOf(this.currentGalaxy);
            }
        }
        return -1;
    }
